...
def create_ui():
    ...
    with gr.Tab('Chat', elem_id='main'):
        ...
        with gr.Row():
            shared.gradio['Stop'] = gr.Button('Parar', elem_id='stop')
            shared.gradio['Generate'] = gr.Button('Generar', elem_id='Generate', variant='primary') ##biroedit
            shared.gradio['Continue'] = gr.Button('Continuar')

        with gr.Row():
            shared.gradio['Impersonate'] = gr.Button('Impersonar')
            shared.gradio['Regenerate'] = gr.Button('Re-generar')
            shared.gradio['Remove last'] = gr.Button('Borrar ultimo', elem_classes=['button_nowrap'])

        with gr.Row():
            shared.gradio['Copy last reply'] = gr.Button('Copiar ultima respuesta')
            shared.gradio['Replace last reply'] = gr.Button('Reemplazar ultima respuesta')
            shared.gradio['Send dummy message'] = gr.Button('Enviar Mensaje Prueba')
            shared.gradio['Send dummy reply'] = gr.Button('Enviar Respuesta Prueba')

        with gr.Row():
            shared.gradio['Clear history'] = gr.Button('Borrar Historial')
            shared.gradio['Clear history-confirm'] = gr.Button('Confirmar', variant='stop', visible=False)
            shared.gradio['Clear history-cancel'] = gr.Button('Cancelar', visible=False)

        with gr.Row():
            shared.gradio['start_with'] = gr.Textbox(label='Empezar respuesta con', placeholder='RSM Bot:', value=shared.settings['start_with'])

        with gr.Row():
            shared.gradio['mode'] = gr.Radio(choices=['chat', 'chat-instruct', 'instruct'], value=shared.settings['mode'] if shared.settings['mode'] in ['chat', 'instruct', 'chat-instruct'] else 'chat', label='Modo', info='Define como se genera el comando del chat. En modo instructor y chat-instructor, la plantilla de instruccion seleccionada en "Configuraciones de Chat" debe corresponder al modelo actual.')
            shared.gradio['chat_style'] = gr.Dropdown(choices=utils.get_available_chat_styles(), label='Estilo de chat', value=shared.settings['chat_style'], visible=shared.settings['mode'] != 'instruct')

    with gr.Tab('Configuración de Chat', elem_id='chat-settings'):
        with gr.Tab("Personaje de IA"):
            with gr.Row():
                with gr.Column(scale=8):
                    ...
                    shared.gradio['name1'] = gr.Textbox(value=shared.settings['name1'], lines=1, label='Tu nombre')
                    shared.gradio['name2'] = gr.Textbox(value=shared.settings['name2'], lines=1, label='IA')
                    shared.gradio['context'] = gr.Textbox(value=shared.settings['context'], lines=10, label='Contexto de IA', elem_classes=['add_scrollbar'])
                    shared.gradio['greeting'] = gr.Textbox(value=shared.settings['greeting'], lines=5, label='Saludo', elem_classes=['add_scrollbar'])

                with gr.Column(scale=1):
                    shared.gradio['character_picture'] = gr.Image(label='Perfil de IA', type='pil')
                    shared.gradio['your_picture'] = gr.Image(label='Tu perfil', type='pil', value=Image.open(Path('cache/pfp_me.png')) if Path('cache/pfp_me.png').exists() else None)

        with gr.Tab("Plantilla de Instrucción"):
            with gr.Row():
                with gr.Row():
                    shared.gradio['instruction_template'] = gr.Dropdown(choices=utils.get_available_instruction_templates(), label='Plantilla de Instruccion', value='None', info='Cambiar esto de acuerdo al modelo/LoRA que estás utilizando. Usado en modo instructor y chat-instructor.', elem_classes='slim-dropdown')
            ...
            shared.gradio['name1_instruct'] = gr.Textbox(value='', lines=2, label='Usuario string')
            shared.gradio['name2_instruct'] = gr.Textbox(value='', lines=1, label='Bot string')
            shared.gradio['context_instruct'] = gr.Textbox(value='', lines=4, label='Contexto')
        ...
        with gr.Tab('Subir personaje de IA'):
